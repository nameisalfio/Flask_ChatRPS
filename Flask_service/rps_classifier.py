...
def load_model(model_path):
    model = RPSClassifier()  # Definizione del modello
    model_weights = torch.load(model_path, map_location=torch.device('cpu'))

    # Aggiungi il prefisso "model." a ogni chiave del state_dict
    new_model_weights = OrderedDict()
    for k, v in model_weights.items():
        new_key = "model." + k
        new_model_weights[new_key] = v

    # Carica il modello con le chiavi modificate
...
def predict(image_path, model):
    ...
    # Sposta l'immagine sul dispositivo corretto
